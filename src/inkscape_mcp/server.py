...
# E19-02: `instructions` delivers a concise system overview IN-CONTEXT every turn (the document
...
def register_tools() -> None:
    ...
    the full E1 surface onto the server at boot.
    """
    ...
    # E1 resources, then tools — self-register via @mcp.resource / @mcp.tool on import.
    # MCP Prompts (architecture §4.1). The E8-05 `live_canvas_assist` Prompt orients the agent on
    # the live-view perceive→decide→act→observe loop; the E5-07 export/recolor library
    # (`prepare_web_export` / `prepare_icon_set` / `prepare_print_export` / `theme_recoloring`)
    # orients it on the shipped export + recolor surface; the E15-04 authoring library
    # (`compose_artwork` / `restyle_artwork`) on-ramps the E14 generative create→draw→render→export
    ...
    # E16-10e: the `inkscape://prompts` index resource surfaces the registered prompts through the
    ...
    # E17-01: post-registration pass — stamp every registered tool with its MCP `ToolAnnotations`
    ...
    # E20-03: final post-registration pass — when `INKSCAPE_MCP_TOOL_DESC=short`, replace each
    ...
    # E17-02: after annotations, a second central pass stamps every tool with its {domain, risk}
    ...
    # E1 tools plus the E2 safe-edit tools (style / text+object / transform), save-as, and export
    # profiles, the E3 live-mode tools (`live`, gate on by default), the E6 HIGH-risk path
    # geometry tools (`paths`, dry-run + approval-gated), and the E6-02 controlled Action surface
    # (`actions`: discovery + allowlist + versioned map + validated chains, exec HIGH + approval-
    # gated, plus the E6-03 ADR-003 `run_raw_action` escape hatch — advanced-mode/OFF-by-default,
    ...
    # The E5 convenience/polish surface (E5-04 `svg_web_optimize` — reversible web optimization;
    # E5-05 `quality_report` — read-only machine-readable quality metrics; E5-06 `export_batch` —
    # bounded, dry-run-default typed batch export) rides the same pipelines, no new authority.
    # E14-08 adds the read-only `discover.how_do_i` intent→tool discoverability tool (guidance
    ...
    # E16-08 adds the HIGH-risk, reversible `delete_object` in the `dom` structural-edit module
    # (snapshot + Operation Record per op via the shared pipeline; approval-gated).
    # E19-01 adds the `batch.apply_edits` tool: N typed DOM edits (a discriminated union over the
    ...
    # E20-02 adds the `transform_objects` tool: a declarative SELECTOR (the find_objects predicate
